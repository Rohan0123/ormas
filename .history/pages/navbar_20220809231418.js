        ...
        <ol className='list-decimal px-8'>
          ...
          {Object.keys(cart).map((k)=>{return <li key={k}>
            <div className='item flex'>
              ...
              <AiFillMinusCircle onClick={()=>{removeFromCart(k, 1, cart[k].price, cart[k].name, cart[k].size, cart[k].variant)}} className='cart_buttons text-2xl hover:'/>{cart[k].qty}<AiFillPlusCircle onClick={()=>{addToCart(k, 1, cart[k].price, cart[k].name, cart[k].size, cart[k].variant)}}className='cart_buttons text-2xl'/>
              </div>
            </div>
          </li>})}
        </ol>
