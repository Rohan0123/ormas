...
const Post = ({ addToCart, buyNow, product, variants, saveCart, clearCart }) => {
        ...
        <section class="text-gray-400 bg-black body-font overflow-hidden" id={styles.details}>
          <div class="container px-5 py-16 mx-auto">
            <div class="lg:w-4/5 mx-auto flex flex-wrap">
              <img alt="ecommerce" class="lg:w-1/2 w-full lg:h-auto h-64 object-cover object-center rounded-xl" src={product.img} />
              <div class="lg:w-1/2 lg:pl-10 lg:py-6 mt-6 lg:mt-0 " >
                ...
                <div class="flex mt-6 items-center pb-5 border-b-2 border-gray-800 mb-5">
                  <div class="flex">
                    {!Object.keys(variants).includes('') && <span class="mr-2">Color</span>}

                    {Object.keys(variants).includes("white") && Object.keys(variants['white']).includes(size) && <button onClick={()=>{refreshVariants(size, 'white')}} class={`border-2 border-gray-800 rounded-full w-6 h-6 focus:outline-none bg-white mr-2 ${color === 'white'? 'border-black': 'border-gray-300'}`}></button>}
                    {Object.keys(variants).includes('red') && Object.keys(variants['red']).includes(size) && <button onClick={()=>{refreshVariants(size, 'red')}} class={`border-2 border-gray-800 ml-1  rounded-full w-6 h-6 focus:outline-none bg-red-600 mr-2 ${color === 'red'? 'border-black': 'border-gray-300'}`}></button>}
                    {Object.keys(variants).includes('blue') && Object.keys(variants['blue']).includes(size) && <button onClick={()=>{refreshVariants(size, 'blue')}} class={`border-2 border-gray-800 ml-1 rounded-full w-6 h-6 focus:outline-none bg-blue-900 mr-2 ${color === 'blue'? 'border-black': 'border-gray-300'}`}></button>}
                    {Object.keys(variants).includes('green') && Object.keys(variants['green']).includes(size) && <button onClick={()=>{refreshVariants(size, 'green')}} class={`border-2 border-gray-800 ml-1 rounded-full w-6 h-6 focus:outline-none bg-green-600 mr-2 ${color === 'green'? 'border-black': 'border-gray-300'}`}></button>}
                    {Object.keys(variants).includes('black') && Object.keys(variants['black']).includes(size) && <button onClick={()=>{refreshVariants(size, 'black')}} class={`border-2 border-gray-800 ml-1  rounded-full w-6 h-6 focus:outline-none bg-black mr-2 ${color === 'black'? 'border-black': 'border-gray-300'}`}></button>}
                  </div>
                  <div class="flex items-center">
                    <span class="mr-3">Size</span>
                    <div class="relative">
                      <select value={size} onChange={(e)=>refreshVariants(e.target.value, color)} class="rounded border border-gray-700 focus:ring-2 focus:ring-yellow-900 bg-transparent appearance-none py-2 focus:outline-none focus:border-yellow-500 text-orange-200 pl-3 pr-10">
                        {Object.keys(variants[color]).includes('50ml') && <option className='bg-black hover:bg-orange-200 hover:text-black'  value={'50ml'}>50ml</option>}
                        {Object.keys(variants[color]).includes('100ml') && <option className='bg-black hover:bg-orange-200 hover:text-black' value={'100ml'}>100ml</option>}
                        {Object.keys(variants[color]).includes('250ml') && <option className='bg-black hover:bg-orange-200 hover:text-black' value={'250ml'}>250ml</option>}
                        {Object.keys(variants[color]).includes('50gm') && <option className='bg-black hover:bg-orange-200 hover:text-black' value={'50gm'}>50gm</option>}
                ...
                <div class="flex">
                  <span class="title-font font-medium text-2xl text-white">₹{product.price}</span>
                    ...
                    <button onClick={() => {buyNow(slug, 1, product.price, product.title, product.size, product.color) }} class="flex ml-auto text-black bg-orange-200 border-0 py-2 px-6 focus:outline-none hover:bg-orange-100 rounded">Buy Now</button>
                  
                  <button onClick={() => { addToCart(slug, 1, product.price, product.title, product.size, product.color) }} className='hover:cursor-pointer text-3xl mx-5'><AiOutlineShoppingCart/></button>
